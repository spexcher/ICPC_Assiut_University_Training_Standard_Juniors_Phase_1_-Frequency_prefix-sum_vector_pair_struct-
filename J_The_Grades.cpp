...
{
  int t1 = a.a + a.m + a.s + a.e;
  int t2 = b.a + b.m + b.s + b.e;
  if (t1 == t2)
...
void solve()
{
  ...
  {
    cin >> name >> a >> m >> s >> e;
    v.push_back({name, a, m, s, e});
  }
  sort(all(v), compareFriend);
  fo(i, n)
  {
    int total = v[i].a + v[i].m + v[i].s + v[i].e;
    print(v[i].name << sp << total << sp << v[i].a << sp << v[i].m << sp << v[i].s << sp << v[i].e);
  }
}